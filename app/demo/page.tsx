    ...
    const sendMessage = async (messageType: string) => {
      ...
      switch (messageType) {
        case "ReadyForPickup":
          ...
          break;
        case "Reminder":
          predefinedMessage = "Friendly reminder about your dry cleaning ready for pickup.";
          break;
        case "DiscountOffer":
          predefinedMessage = "Special discount offer just for you! 10% off for your next visit.";
          break;
        default:
          break;
      }
      ...
      const res = await fetch('/api/sendSMS', {
        ...
        body: JSON.stringify({ phone: phone, message: predefinedMessage }),
      });
  ...
  return (
    <div className="w-full flex justify-center">
      ...
      <Image className="w-20 h-20 z-20" src={logo} alt="" />
        ...
        <button className="button" disabled={loading} onClick={() => sendMessage("ReadyForPickup")}>
            Ready for Pickup
          </button>
          <button className="button" disabled={loading} onClick={() => sendMessage("Reminder")}>
            Reminder
          </button>
          <button className="button" disabled={loading} onClick={() => sendMessage("DiscountOffer")}>
            Discount Offer
          </button>
          {success && (
